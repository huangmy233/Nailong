...
best record is kept in that browser's local storage.
...
The best result is stored next to the game (next to the .exe for the packaged
build) in **`records.json`**:
...
A faster time always wins; if two times are equal, fewer flips wins. If the
file is missing, empty or damaged, the game simply starts with no record - it
never crashes because of it. Delete the file to reset your record.
...
nailong_memory/
    ...
    build_web.py        builds web/index.html
...
the platform forces them: the best record lives in the browser's local storage
...
5. Finish the pairs and show the victory screen with the time, the flip count
   and the saved best record.
