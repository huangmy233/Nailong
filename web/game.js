...
(function () {
  ...
  // ------------------------------------------------------------ records -----
  /* Two independent leaderboards, exactly as records.py keeps them:
       time   fastest completion, an equal time decided by fewer flips
       flips  fewest flips, an equal flip count decided by the faster time */
  var TIME_EPSILON = 0.005;

  function emptyRecord() { return { time: null, flips: null }; }

  function hasAnyRecord(record) {
    return !!(record && (record.time || record.flips));
  }

  function entry(data) {
    if (!data || typeof data !== "object") { return null; }
    var t = Number(data.time), f = Math.round(Number(data.flips));
    if (!isFinite(t) || !isFinite(f) || t <= 0 || f <= 0) { return null; }
    return { time: t, flips: f };
  }
  ...
  function loadRecord() {
    ...
    if (!data || typeof data !== "object") { return emptyRecord(); }
    if (typeof data.best_time === "number") {        // the old one-board format
      var old = entry({ time: data.best_time, flips: data.best_flips });
      return { time: old, flips: old ? { time: old.time, flips: old.flips }
                                     : null };
    }
    return { time: entry(data.best_time), flips: entry(data.best_flips) };
  }

  function dumpEntry(e) {
    return e ? { time: Math.round(e.time * 100) / 100, flips: e.flips } : null;
  }

  function saveRecord(record) {
    try {
      window.localStorage.setItem(RECORD_KEY, JSON.stringify({
        best_time: dumpEntry(record.time),
        best_flips: dumpEntry(record.flips)
      }));
    } catch (e) { /* private mode: play on without saving */ }
  }

  function beatsTime(t, flips, e) {
    if (!e) { return true; }
    if (t < e.time - TIME_EPSILON) { return true; }
    if (Math.abs(t - e.time) <= TIME_EPSILON) { return flips < e.flips; }
    return false;
  }

  function beatsFlips(t, flips, e) {
    if (!e) { return true; }
    if (flips !== e.flips) { return flips < e.flips; }
    return t < e.time - TIME_EPSILON;
  }

  function submitRun(t, flips, record) {
    var next = { time: record && record.time ? record.time : null,
                 flips: record && record.flips ? record.flips : null };
    var improved = [];
    if (beatsTime(t, flips, next.time)) {
      next.time = { time: t, flips: flips };
      improved.push("time");
    }
    if (beatsFlips(t, flips, next.flips)) {
      next.flips = { time: t, flips: flips };
      improved.push("flips");
    }
    if (improved.length) { saveRecord(next); }
  ...
  var G = {
    ...
    toasts: [], confirmRestart: false, improved: [],
  ...
  function layout() {
    ...
    G.victory = { x: cx - vw2 / 2, y: h / 2 - 172 * s, w: vw2, h: 344 * s };
  ...
  function win() {
    ...
    stopLaugh();
    playSound("victory");
    var result = submitRun(G.elapsed, G.flips, G.record);
    G.record = result.record;
    G.improved = result.improved;
  }

  function recordLine() {
    var parts = [];
    if (G.record.time) {
      parts.push("Best time " + formatTime(G.record.time.time));
    }
    if (G.record.flips) {
      parts.push("Fewest flips " + G.record.flips.flips);
    }
    return parts.join("   |   ");
  }
  ...
  function drawTitle() {
    ...
    if (hasAnyRecord(G.record)) {
      text(recordLine(), 20 * s, C.GREEN_DARK, "midleft", 26 * s, tl.bottomY,
           true);
    }
  ...
  function drawHud() {
    ...
    var best = G.record.time ? formatTime(G.record.time.time) : NO_TIME;
    var bestFlips = G.record.flips ? String(G.record.flips.flips) : "--";
    var stats = [
  ...
  function drawVictory() {
    ...
    textFitted("YOU BEAT THE MAYHEM!", 44 * s, C.GREEN_DARK,
               { x: p.x, y: p.y + 52 * s + bounce - 5, w: p.w, h: 10 }, true);
    text("Time " + formatTime(G.elapsed) + "   -   " + G.flips + " flips",
         30 * s, C.INK, "center", cx, p.y + 100 * s, true);

    var y = p.y + 134 * s;
    if (G.improved.length) {
      var banner = G.improved.length === 2
        ? "NEW BEST TIME  +  NEW FEWEST FLIPS!"
        : (G.improved[0] === "time" ? "NEW BEST TIME!" : "NEW FEWEST FLIPS!");
      textFitted(banner, 28 * s, C.PINK_DARK,
                 { x: p.x, y: y - 5, w: p.w, h: 10 }, true);
      y += 34 * s;
    }
    [["Best time", "time", C.BLUE_DARK],
     ["Fewest flips", "flips", C.PINK_DARK]].forEach(function (it) {
      var e = G.record[it[1]];
      var line = it[0] + "  -";
      if (e) {
        line = it[1] === "time"
          ? it[0] + "  " + formatTime(e.time) + "  in " + e.flips + " flips"
          : it[0] + "  " + e.flips + " flips  in " + formatTime(e.time);
      }
      text(line, 21 * s, it[2], "center", cx, y, false);
      y += 26 * s;
    });
    text("7 / 7 pairs  +  both Nailongs found", 20 * s, C.INK_SOFT, "center",
         cx, y + 2 * s, false);
  ...
  window.NAILONG_STATE = function () {
    ...
    return {
      ...
      confirm: G.confirmRestart, improved: G.improved, record: G.record,
